...
** Setup the desktop to make life easier still
   ...
   5. Add extra desktops
      ...
      + Make 4 desktops (this should make a desktop switching widget \
        on the toolbar)
...
** Setup basic DAQ user
   1. Make new users (here I'll make the DAQ user: "daq")
   ...
   6. Make the data partition owned appropriately
      #+BEGIN_SRC sh
      cd /
      sudo chown -R daq:daq data
      cd
      #+END_SRC
   7. Make Downloads folder in the storage area and link everything
      #+BEGIN_SRC sh
      cd
      ln -s /data data
...
** Install other things by hand
   ...
   - MIDAS
     ...
     http://engesrv.physics.ncsu.edu/Docs/MIDAS
